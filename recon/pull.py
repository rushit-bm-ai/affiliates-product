"""Metabase data pull + file validation — recon tab."""
...
def authenticate():
    ...
    token = resp.json()["id"]
    print(f"[pull] Authenticated with Metabase (token: {token[:8]}…)")
    return token
...
def pull_native_query(token, query_config):
    ...
    print(f"[pull] Running {os.path.basename(sql_file)} → {os.path.basename(outfile)} …")
    ...
    print(f"[pull]   ✓ {len(rows)} rows written")
    return len(rows)
...
def main(skip_pull=False):
    ...
    if skip_pull:
        print("[pull] --skip-pull: using cached CSVs")
    else:
        token = authenticate()
        for name, qcfg in config.QUERIES.items():
            try:
                ...
                print(f"[pull] ERROR pulling {name}: {e}")
    ...
    for name, qcfg in config.QUERIES.items():
        ...
        print(f"[pull] {status_icon} {vr['file']}: {vr['row_count']} rows, {vr['status']}")
    ...
    print(f"[pull] Pull log written → {os.path.basename(log_path)}")
    return pull_log
