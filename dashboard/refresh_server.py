"""Lightweight HTTP API that triggers pipeline refreshes from the dashboard UI.

Endpoints:
  POST /api/refresh         — start a recon pipeline run (202 started / 409 already_running)
  GET  /api/refresh-status  — current status (idle / running / success / error)
  POST /api/refresh-c1b     — start experiments (C1B) pipeline run
  GET  /api/c1b-refresh-status — C1B pipeline status
...
STATUS_FILE      = os.path.join(PROJECT_ROOT, "computed", "refresh_status.json")
C1B_STATUS_FILE  = os.path.join(PROJECT_ROOT, "computed", "c1b_refresh_status.json")
C1B_SCRIPT       = os.path.join(PROJECT_ROOT, "experiments", "run.py")
PORT = 8765
